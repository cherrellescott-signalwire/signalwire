...
sw_bulk_number_get_list () {
    ...
    FIRST_PAGE=$(curl "https://$SW_SPACE_URL/api/laml/2010-04-01/Accounts/$SW_PROJ_KEY/IncomingPhoneNumbers.json?PageSize=$MANY" \
...
sw_bulk_number_delete () {
    while [ "${1:-}" != "" ]; do
	case ${1:-} in
	    (-h|--help)  printf "./$0 --action=dry|delete --many=[1 thru 1000] --pick=[all|800,888...] [--debug]\n\n"; exit;;
	...
	if [ $ACTION == delete ]; then
	    curl https://$SW_SPACE_URL/api/laml/2010-04-01/Accounts/$SW_PROJ_KEY/IncomingPhoneNumbers/$sid.json \
	         -X DELETE -u "$SW_PROJ_KEY:$SW_TOKEN"
